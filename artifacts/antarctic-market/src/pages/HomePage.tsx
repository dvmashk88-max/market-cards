...
import {
  ...
  CheckCircle2, Zap, Shield, Globe, ArrowRight, Sparkles, TriangleAlert,
} from "lucide-react";
...
function ProductCard({ p, selected, onSelect }: { p: Product; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <div
      onClick={onSelect}
      className="relative cursor-pointer overflow-hidden rounded-2xl transition-[transform,box-shadow,border-color,background-color] duration-200 hover:-translate-y-0.5"
      style={selected
...
function OrderPanel({ prod, selectedOfferId, onOffer, nominalsExpanded, onToggleNominals, email, onEmail, checkoutPending, checkoutError, onCheckout }: {
  ...
  return (
    <div className="rounded-[26px] bg-gradient-to-r from-purple-500/70 via-indigo-400/45 to-cyan-400/70 p-px shadow-[0_14px_34px_rgba(8,12,34,0.34)]">
      <div className="overflow-hidden rounded-[25px] bg-[#090d20]">
...
export default function HomePage() {
  ...
  const categoriesQuery = useQuery({
    ...
    retry: shouldRetryStorefrontQuery,
  });
  ...
  return (
    <div
      className="min-h-screen overflow-x-hidden bg-[#050818] font-sans text-white"
      style={{
        backgroundImage:
          "radial-gradient(circle at 18% 8%,rgba(109,40,217,0.18),transparent 32%),radial-gradient(circle at 88% 62%,rgba(6,182,212,0.10),transparent 30%)",
      }}
    >
      ...
      <main>
        ...
        <section className="relative min-h-[88vh] flex flex-col items-center justify-center text-center px-4 pt-24 pb-10 overflow-hidden">
          <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col items-center space-y-7">
            ...
            <h1
              ...
              style={{ fontSize:"clamp(2.6rem, 7vw, 5.5rem)", textShadow:"0 8px 32px rgba(43,26,95,0.38)" }}
            >
            ...
            <div className="flex flex-col gap-3 lg:flex-row">
              <div className="grid w-full grid-cols-1 gap-3 sm:w-auto sm:grid-cols-2">
                <a
                  ...
                  style={{ background:"linear-gradient(90deg,#7c3aed,#06b6d4)", boxShadow:"0 10px 24px rgba(22,20,64,0.34)", textDecoration:"none" }}
                >
        ...
        <section id="directions" aria-labelledby="catalog-directions-title" className="px-4 py-10">
          <div className="mx-auto max-w-6xl rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:p-7">
            ...
            <nav aria-label="Каталог по направлениям" className="mt-5 flex flex-wrap gap-x-5 gap-y-3">
              {catalogSeoPages.map((page) => (
                <a
                  className="text-sm font-semibold text-white/65 underline decoration-white/15 underline-offset-4 transition-colors hover:text-cyan-200 hover:decoration-cyan-300/40"
                  href={page.path}
                  key={page.path}
                >
                  {page.shortName}
